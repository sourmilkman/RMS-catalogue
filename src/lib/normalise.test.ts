...
const headers = ['email', 'name', 'date of birth', 'Age if you are aged between 14-30', 'artwork 1 (image attachment)', 'title of artwork 1', 'medium', 'votes - artwork 1', 'artwork 2 (image attachment)', 'title of artwork 2', 'medium', 'votes - artwork 2', 'artwork 3 (image attachment)', 'title of artwork 3', 'medium', 'votes - artwork 3', 'artwork 4 (image attachment)', 'title of artwork 4', 'medium', 'votes - artwork 4']
const csv = [
  headers,
  ['a@example.com', 'Alice Example', '', '17', 'https://example.com/one.jpg', 'First', 'Oil', 'Yes: 5; Maybe: 2; No: 0', '', '', '', '', '', '', '', '', 'https://example.com/four.jpg', 'Fourth', 'Ink', 'Yes: 1; Maybe: 2; No: 2'],
...
describe('sheet normalisation', () => {
  it('discovers repeating artwork groups and tolerates missing slots', () => {
    ...
    expect(result.artists[0].youngArtistAge).toBe(17)
    expect(result.artists[0].warnings.some((warning) => warning.code === 'missing-dob')).toBe(false)
